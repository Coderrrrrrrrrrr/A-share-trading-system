...
def find_consecutive_rising_stocks():
    """查找连续7天或以上上涨的股票"""
    try:
        # 创建数据库连接
        ...
        # 查询所有股票的交易数据
        query = """
        SELECT stock_name, stock_code, trade_date, change_percent
        FROM stock_data 
        ...
        for stock_code, group in df.groupby('stock_code'):
            ...
            if max_consecutive_days >= 7:
                ...
                print(f"股票 {stock_name}({stock_code}) 连续上涨 {max_consecutive_days} 天，起始日期: {start_date}")
...
def plot_candlestick_chart(stock_name, stock_code):
    """为指定股票绘制蜡烛图"""
    try:
        # 创建数据库连接
        engine = create_engine(f"mysql+pymysql://{DB_CONFIG['user']}:{DB_CONFIG['password']}@{DB_CONFIG['host']}/{DB_CONFIG['database']}?charset={DB_CONFIG['charset']}")
...
if __name__ == "__main__":
    ...
    # rising_stocks = find_consecutive_rising_stocks()
    
    # if rising_stocks:
    #     print(f"\n找到 {len(rising_stocks)} 只连续上涨的股票，开始绘制蜡烛图...")
    #     # 为每只符合条件的股票绘制蜡烛图
    #     for stock in rising_stocks:
    #         print(f"\n正在绘制 {stock['stock_name']}({stock['stock_code']}) 的蜡烛图...")
    #         plot_candlestick_chart(stock['stock_name'], stock['stock_code'])
    # else:
    #     print("\n没有找到连续7天或以上上涨的股票")
